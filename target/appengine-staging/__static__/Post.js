export default function Post({currentProfil, userId}) {
	...
	React.useEffect( () => {
		fetch("http://localhost:8080/_ah/api/myApi/v1/userPost/"+currentProfil)
			.then((response) => {
		...
		fetch("http://localhost:8080/_ah/api/myApi/v1/userLikes/"+userId)
			.then((response) => {
				if(response.status != 200) {
					return;
				}
	...
	function handleLike(key, i) {
		fetch("http://localhost:8080/_ah/api/myApi/v1/likePost/"+key)
			.then((response) => {
				if(response.status != 200) {
					return;
				}
	...
	function handleDislike(key, i) {
		fetch("http://localhost:8080/_ah/api/myApi/v1/unlikePost/"+key)
			.then((response) => {
				if(response.status != 200) {
					return;
				}
